...
# ----------------------------
# 1) Very simple character-level tokenizer
# ----------------------------
class CharTokenizer:
    """
    Character-level tokenizer:
    each character is treated as a token.
    The vocabulary is built from the training texts.
    """

    def __init__(self, texts: List[str]):
        chars = sorted(list(set("".join(texts))))
        self.stoi = {ch: i for i, ch in enumerate(chars)}   # string -> id
        self.itos = {i: ch for ch, i in self.stoi.items()}  # id -> string
...
def make_training_samples(pairs: List[Tuple[str, str]]) -> List[str]:
    ...
    A delimiter ### is added so the model knows where generation should stop.
    """
    samples = []
    for mot, phrase in pairs:
        samples.append(f"Mot: {mot}\nPhrase: {phrase}\n###\n")
    return samples
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # Convert into multi-head format: (B, n_heads, T, head_dim)
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # Residual connection after feedforward layer
...
class TinyGPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        targets : (B, T) labels for the next token
        """
        B, T = idx.shape
...
def build_stream(samples: List[str]) -> str:
    # Concatenate all samples into one long training stream
    return "".join(samples)


def get_batch(data_ids: torch.Tensor, block_size: int, batch_size: int, device: str):
    """
    Randomly select starting positions in the token stream.
...
def main():
    ...
    # --- Small French dataset ---
    pairs = [
        ("pomme", "Une pomme tombe de l'arbre quand elle est mûre."),
        ("gravité", "La gravité attire les objets vers la Terre."),
        ("télescope", "Un télescope permet d'observer les étoiles lointaines."),
        ("océan", "L'océan couvre une grande partie de la surface de la Terre."),
        ("électricité", "L'électricité alimente de nombreux appareils du quotidien."),
        ("lune", "La Lune tourne autour de la Terre et influence les marées."),
        ("pluie", "La pluie se forme lorsque la vapeur d'eau se condense."),
        ("volcan", "Un volcan entre en éruption lorsque le magma remonte à la surface."),
        ("soleil", "Le Soleil fournit la lumière et la chaleur nécessaires à la vie."),
        ("vent", "Le vent déplace l'air d'une région à une autre."),
    ]
    ...
    # --- Demo generation ---
    def run_query(mot: str):
        prompt = f"Mot: {mot}\nPhrase: "
        idx = torch.tensor([tokenizer.encode(prompt)], dtype=torch.long).to(device)

        out = model.generate(idx, max_new_tokens=200, temperature=0.9, top_k=0)
        decoded = tokenizer.decode(out[0].tolist())

        # Stop generation at delimiter ###
        stop = "\n###"
        if stop in decoded:
